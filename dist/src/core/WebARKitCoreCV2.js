...
export class WebARKitCoreCV {
    ...
    track(msg) {
        ...
        if (buf.length !== expectedLen) {
            ...
            console.warn(`ImageData buffer length (${buf.length}) does not match 4*width*height (${expectedLen}). Falling back to derived dimensions.",`, { bufLen: buf.length, width, height });
            ...
            let finalBuf;
            if (buf.length > derivedExpected)
                finalBuf = buf.slice(0, derivedExpected);
